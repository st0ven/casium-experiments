...
*/
export function reorderList(model, { direction }) {
  ...
  const currentOrder =
    order && order.length ? order : Object.keys(items).map((n) => parseInt(n));
...
*/
export function resetList(model) {
  return {
    ...model,
    order: undefined,
    selectedItems: [],
...
*/
export function toggleItemSelection(model, { event, value }) {
